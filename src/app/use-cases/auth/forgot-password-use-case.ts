...
class ForgotPasswordUseCase implements ForgotPassword {
  ...
  public execute = async (email: string) => {
    try {
      ...
      if (updated) {
        // Enqueue the job to send the email
        const { status, data, message } = await this.queueManager.add('ForgotPasswordEmail', { email, token })

        if (!status) throw new AppError(message!)
